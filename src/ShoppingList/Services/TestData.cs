...
namespace ShoppingList.Services {
    public class TestData {
        ...
        public void InsertTestData(IRepository target) {
            ...
            var sl1 = _shoppingListFactory.Create(stores[0], items.TakeRandom(5, 15).ToArray(), new Guid("{E753B7F9-152F-4C2C-ACF4-C6D12F3460C3}"));
            var sl2 = _shoppingListFactory.Create(stores[1], items.TakeRandom(4, 8).ToArray(), new Guid("{E02FE3C4-A742-4054-9A1D-D9904C8375BA}"));
            var sl3 = _shoppingListFactory.Create(stores[2], items.TakeRandom(3, 10).ToArray(), new Guid("{D7175B1A-FA03-4FA0-A6DF-44DAB79BF1F4}"));
            foreach(var item in sl3.Items)             {
                item.IsBought = true;
            }
